...
# The Labus module in this file is used to parse /etc/labus/labus.rb.
#
# salsa: do we need this ?????????????
# Warning to the reader:
# Because the Ruby DSL in /etc/labus/labus.rb does not accept hyphens in
# section names, this module translates names like 'labus_redmine' to the
# correct 'labus-redmine' in the `generate_hash` method. This module is the only
# place in the cookbook where we write 'labus_redmine'.
...
module Labus
  ...
  labus_redmine Mash.new
  gitlab_shell Mash.new
  unicorn Mash.new
  redmine_unicorn Mash.new
  nginx Mash.new
  ci_nginx Mash.new
  logging Mash.new
  ...
  class << self
    ...
    def generate_secrets(node_name)
      SecretsHelper.read_labus_secrets

      # Note: If you add another secret to generate here make sure it gets written to disk in SecretsHelper.write_to_labus_secrets
      Gitlab['gitlab_shell']['secret_token'] ||= generate_hex(64)
      Gitlab['gitlab_rails']['secret_token'] ||= generate_hex(64)
      Labus['labus_redmine']['secret_token'] ||= generate_hex(64)

      # Note: Besides the section below, gitlab-secrets.json will also change
      # in CiHelper in libraries/helper.rb
      SecretsHelper.write_to_labus_secrets
    end
