...
define :labus_unicorn_service, :rails_app => nil, :user => nil do
  rails_app = params[:rails_app]
  rails_home = node['labus'][rails_app]['dir']
  svc = params[:name]
  user = params[:user]
  ...
  unicorn_listen_socket = node['labus'][svc]['socket']
  unicorn_pidfile = node['labus'][svc]['pidfile']
  unicorn_log_dir = node['labus'][svc]['log_directory']
  ...
  unicorn_listen_tcp = [node['labus'][svc]['listen'], node['labus'][svc]['port']].join(':')

  unicorn_rb = File.join(unicorn_etc_dir, "unicorn.rb")
  unicorn_config unicorn_rb do
    listen(
      unicorn_listen_tcp => {
        :tcp_nopush => node['labus'][svc]['tcp_nopush']
      },
      unicorn_listen_socket => {
        :backlog => node['labus'][svc]['backlog_socket'],
      }
    )
    worker_timeout node['labus'][svc]['worker_timeout']
    working_directory unicorn_working_dir
    worker_processes node['labus'][svc]['worker_processes']
    preload_app true
  ...
  runit_service svc do
    ...
    log_options node['labus']['logging'].to_hash.merge(node['labus'][svc].to_hash)
  end
